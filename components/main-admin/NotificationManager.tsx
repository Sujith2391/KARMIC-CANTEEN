...
const NotificationManager: React.FC = () => {
  ...
  const [target, setTarget] = useState<'all' | 'office_only'>('all');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await sendNotification({ title, message, requiresAction, target });
      setTitle('');
      setMessage('');
  ...
  const handleAnnounceEvent = () => {
    setTitle('🎉 Special Announcement');
    setMessage('Hi Team, we are excited to announce a special event! Please join us for...');
    setRequiresAction(false);
    setTarget('all');
  };
      ...
      <div>
        ...
        <form onSubmit={handleSubmit} className="bg-slate-50 p-6 rounded-lg border space-y-4">
          ...
          <div>
            ...
            <div className="mt-2 flex space-x-4">
                <label className="flex items-center">
                    <input type="radio" name="target" value="all" checked={target === 'all'} onChange={() => setTarget('all')} className="h-4 w-4 text-primary border-slate-300 focus:ring-primary"/>
                    <span className="ml-2 text-sm text-slate-700">All Employees</span>
                </label>
                <label className="flex items-center">
                    <input type="radio" name="target" value="office_only" checked={target === 'office_only'} onChange={() => setTarget('office_only')} className="h-4 w-4 text-primary border-slate-300 focus:ring-primary"/>
                    <span className="ml-2 text-sm text-slate-700">Employees in Office (Tomorrow)</span>
                </label>
            </div>
          </div>
          ...
          <button type="submit" disabled={isSubmitting} className="w-full py-2 bg-primary text-onPrimary font-semibold rounded-lg hover:bg-primary-dark disabled:bg-slate-400">
            {isSubmitting ? 'Sending...' : `Send to ${target === 'all' ? 'All Employees' : 'In-Office Employees'}`}
          </button>
        </form>
        ...
        <div className="mt-8">
            ...
            <div className="bg-slate-50 p-6 rounded-lg border space-y-6">
                <div>
                    <h4 className="font-semibold text-slate-800">Announce Function / Event</h4>
                    <p className="text-sm text-slate-600 mb-3">Pre-fills the form above with a template for a special event, targeting all employees.</p>
                    <button 
                        onClick={handleAnnounceEvent} 
                        className="w-full py-2 flex items-center justify-center bg-blue-500 text-onPrimary font-semibold rounded-lg hover:bg-blue-600"
                    >
